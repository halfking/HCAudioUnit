...
static OSStatus channelAudioProducer(void *userInfo, AudioBufferList *audio, UInt32 *frames) {
    ...
    for ( int i=channel->callbacks.count-1, filterIndex=0; i>=0; i-- ) {
        ...
        if ( callback->flags & kFilterFlag ) {
            if ( filterIndex == arg->nextFilterIndex ) { //处理紧临的一个过滤器
                //                printf("load filter %d\n",filterIndex);
                // Run this filter
                channel_producer_arg_t filterArg = *arg;
                filterArg.nextFilterIndex = filterIndex+1;
                status = ((AEAudioFilterCallback)callback->callback)((__bridge id)callback->userInfo, (__bridge AEAudioController *)channel->audioController, &channelAudioProducer, (void*)&filterArg, &arg->timeStamp, *frames, audio);
                //                printf("load filter %d completed\n",filterIndex);
                return status;
    ...
    if ( channel->type == kChannelTypeChannel ) {
        ...
        //        printf("**call back for aeaudioplayable...\n");
...
- (void)dealloc {
    ...
    AECheckOSStatus(AudioUnitRemovePropertyListenerWithUserData(_ioAudioUnit, kAudioUnitProperty_StreamFormat, audioUnitStreamFormatChanged, (__bridge void*)self),
                    "AudioUnitRemovePropertyListener(kAudioUnitProperty_StreamFormat)");
    ...
    __AEAllocated = NO;
    NSLog(@"audio controller released.");
}
    ...
    if ( _inputEnabled ) {
        if ( [audioSession respondsToSelector:@selector(requestRecordPermission:)] ) {
            [audioSession requestRecordPermission:^(BOOL granted) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if ( granted ) {
                        [self updateInputDeviceStatus:0];
                    } else {
...
//BOOL addOffelineRenderInfo(AEAudioController * inRefCon,callback_t renderInput,SourceAudioBufferData mUserData)
//{
...
//    OSStatus result = AUGraphSetNodeInputCallback(THIS->AUGraph, THIS->_topChannel, 0, &rcbs);
//    if (result) { printf("AUGraphSetNodeInputCallback result %ld %08X %4.4s\n", result, (unsigned int)result, (char*)&result); return NO; }
//
...
static void interAppConnectedChangeCallback(void *inRefCon, AudioUnit inUnit, AudioUnitPropertyID inID, AudioUnitScope inScope, AudioUnitElement inElement) {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ( THIS->_inputEnabled ) {
            [THIS updateInputDeviceStatus:0];
        }
    });
}
#endif
...
- (void)updateInputDeviceStatus:(int)deep {
    if ( !_audioGraph || deep > 10) return;
    NSAssert(_inputEnabled, @"Input must be enabled");
    
    if ( _updatingInputStatus ) {
        deep ++;
        // Defer update until prior update complete
        [_messageQueue performAsynchronousMessageExchangeWithBlock:^{} responseBlock:^{ [self updateInputDeviceStatus:deep]; }];
        return;
    }
...
- (AUNode)getMixerNode
{
    return _topGroup->mixerNode;
}
- (AudioUnit)getMixerAudioUnit
{
    return _topGroup->mixerAudioUnit;
}
